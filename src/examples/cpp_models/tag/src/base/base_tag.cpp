...
BeliefUpperBound* BaseTag::CreateBeliefUpperBound(string name) const {
	if (name == "TRIVIAL") {
		...
		cerr << "Unsupported upper bound algorithm: " << name ;
		exit(1);
		return NULL;
	}
}
...
void BaseTag::PrintAction(int action, ostream& out) const {
	switch(action) {
		case 0: out << "South" ; break;
		case 1: out << "East" ; break;
		case 2: out << "North" ; break;
		case 3: out << "West" ; break;
		case 4: out << "Tag" ; break;
		default: out << "Wrong action" ; exit(1);
	}
}
