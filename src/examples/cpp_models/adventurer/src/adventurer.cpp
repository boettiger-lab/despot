...
Adventurer::Adventurer(int num_goals) {
	...
	if (num_goals != 2 && num_goals != 50) {
		cerr << "[Adventurer::Adventurer] Only 2 or 50 goals are supported." ;
		exit(0);
	}
...
ParticleUpperBound* Adventurer::CreateParticleUpperBound(string name) const {
	if (name == "TRIVIAL" || name == "DEFAULT") {
		...
		cerr << "Unsupported particle lower bound: " << name ;
		exit(1);
	}
}
...
ScenarioUpperBound* Adventurer::CreateScenarioUpperBound(string name,
	...
	if (name == "TRIVIAL" || name == "DEFAULT") {
		...
		cerr << "Unsupported scenario upper bound: " << name ;
		exit(1);
		return NULL;
	}
}
...
ScenarioLowerBound* Adventurer::CreateScenarioLowerBound(string name,
	...
	if (name == "TRIVIAL") {
		...
		cerr << "Unsupported scenario lower bound: " << name ;
		exit(-1);
		return NULL;
	}
}
...
void Adventurer::PrintAction(int action, ostream& out) const {
	out << (action == 0 ? "Stay" : ((action == 2) ? "Right" : "Left")) ;
}
	...
	if (type == "MDP") {
		...
		for (int s = 0; s < num_states; s++) {
			...
			/*
			 cout << "State " << s ;
			 PrintState(*states_[s]);
			 cout << "Action " << Compass::CompassString[policy_[s].action] ;
			 */
		}
		cerr << "MDP upper bound " << policy_[0].value ;
	} else {
		cerr << "Unsupported default action type " << type ;
		exit(0);
	}
}
...
POMCPPrior* Adventurer::CreatePOMCPPrior(string name) const {
	if (name == "UNIFORM") {
		...
		cerr << "Unsupported POMCP prior: " << name ;
		exit(1);
		return NULL;
	}
}
...
void Adventurer::PrintPOMDPX() const {
	...
	cout << "</Parameter> </CondProb> </InitialStateBelief>" ;
	...
	cout
		...
		;
	cout << "</Parameter> </CondProb> </StateTransitionFunction>" ;
	...
	cout
		...
		;
	cout << "</Parameter> </CondProb> </ObsFunction>" ;
	...
	cout
		...
		;
	cout << "</Parameter> </Func> </RewardFunction>" ;
	cout << "</pomdpx>" ;
}
