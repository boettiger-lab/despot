...
BaseRockSample::BaseRockSample(string map) {
	...
	/*
	 clock_t start = clock();
	 cerr << "Initializing transitions" ;
	 InitializeTransitions();
	 cerr << "Done " << (double (clock() - start) / CLOCKS_PER_SEC) ;
...
void BaseRockSample::Init_4_4() {
	cout << "Using special layout for rocksample(4, 4)" ;
...
void BaseRockSample::Init_5_5() {
	cout << "Using special layout for rocksample(5, 5)" ;
...
void BaseRockSample::Init_5_7() {
	cout << "Using special layout for rocksample(5, 7)" ;
...
void BaseRockSample::Init_7_8() {
	...
	cout << "Using special layout for rocksample(7, 8)" ;
...
void BaseRockSample::Init_11_11() {
	...
	cout << "Using special layout for rocksample(11, 11)" ;
...
Belief* BaseRockSample::InitialBelief(const State* start, string type) const {
	if (type == "DEFAULT" || type == "PARTICLE") {
		...
		cerr << "Unsupported initial belief type: " << type ;
		exit(0);
	}
}
...
ScenarioLowerBound* BaseRockSample::CreateScenarioLowerBound(string name, string
	...
	if (name == "TRIVIAL") {
		...
		cerr << "Unsupported lower bound algorithm: " << name ;
		exit(0);
		return NULL;
	}
}
...
BeliefLowerBound* BaseRockSample::CreateBeliefLowerBound(string name) const {
	if (name == "TRIVIAL") {
		...
		cerr << "Unsupported belief lower bound: " << name ;
		exit(0);
		return NULL;
	}
}
...
void BaseRockSample::PrintAction(int action, ostream& out) const {
	if (action < E_SAMPLE)
		out << Compass::CompassString[action] ;
	if (action == E_SAMPLE)
		out << "Sample" ;
	if (action > E_SAMPLE)
		out << "Check " << (action - E_SAMPLE - 1) ;
}
