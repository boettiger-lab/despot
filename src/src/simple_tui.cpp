...
Solver *SimpleTUI::InitializeSolver(DSPOMDP *model, string solver_type,
      ...
      solver_type == "PLB") // PLB: particle lower bound
  {
    ...
    logi << "Created lower bound " << typeid(*lower_bound).name() ;
    ...
    logi << "Created POMCP prior " << typeid(*prior).name() ;
    ...
    cerr << "ERROR: Unsupported solver type: " << solver_type ;
    exit(1);
  }
  return solver;
}
...
void SimpleTUI::DisplayParameters(option::Option *options, DSPOMDP *model) {
  ...
  string lbtype = options[E_LBTYPE] ? options[E_LBTYPE].arg : "DEFAULT";
  string ubtype = options[E_UBTYPE] ? options[E_UBTYPE].arg : "DEFAULT";
  // default_out << "Model = " << typeid(*model).name()
  //             << "Random root seed = " << Globals::config.root_seed
  //             << "Search depth = " << Globals::config.search_depth
  //             << "Discount = " << Globals::config.discount
  //             << "Simulation steps = " << Globals::config.sim_len
  ...
  //             << Globals::config.max_policy_sim_len
  //             << "Target gap ratio = " << Globals::config.xi ;
  default_out << "Round;Step;Action;State;Observation;OsProb;Reward;Curr_rewards_discounted;Curr_rewards_undiscounted" << endl ;
  // << "Solver = " << typeid(*solver).name()  ;
}
...
void SimpleTUI::RunEvaluator(DSPOMDP *model, Evaluator *simulator,
  ...
  if (simulator_type == "ippc" && num_runs != 30) {
    cout << "Exit without receiving reward."
         << "Total time: Real / CPU = "
