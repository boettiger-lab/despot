...
POMDPX::POMDPX(string params_file) {
	...
	logi << "#State / log2(#State) = " << parser_->NumStates() << " / "
		<< parser_->LogNumStates() ;
	logi << "#InitState / log2(#InitState) = " << parser_->NumInitialStates()
		<< " / " << parser_->LogNumInitialStates() ;
	logi << "#Action = " << parser_->NumActions() ;
	logi << "#Obs / log2(#Obs) = " << parser_->NumObservations() << " / "
		<< parser_->LogNumObservations() ;
...
void POMDPX::PrintModel(ostream& out) const {
	out << "Parser pointer = " << parser_ ;
	out << "is_small = " << is_small_ ;

	out << "Min reward action = " << min_reward_action_ ;
	out << "Max reward action = " << max_reward_action_ ;

	out << "States = " << states_ ;
	out << "Transitions = " << transition_probabilities_ ;
	out << "Rewards = " << rewards_ ;
	out << "MemoryPool = " << &memory_pool_ ;
}
	...
	void Update(int action, OBS_TYPE obs) {
		...
		for (int i = 0; i < max_iter_; i++) {
			...
			if (updated.size() == particles_.size()) {
				logi << "[POMDPXBelief::Update] " << i
					<< " iterations for normal simulations" ;
				break;
			}
		}
		...
		if (updated.size() != particles_.size()) {
			for (int i = 0; i < max_iter_ + 1; i++) {
				...
				if (updated.size() == particles_.size()) {
					logi << "[POMDPXBelief::Update] " << i
						<< " iterations for noisy simulations" ;
					break;
				}
			}
		}
...
ParticleUpperBound* POMDPX::CreateParticleUpperBound(string name) const {
	...
	if (name == "TRIVIAL") {
		...
		cerr << "Unsupported particle upper bound: " << name ;
		exit(1);
		return NULL;
	}
}
