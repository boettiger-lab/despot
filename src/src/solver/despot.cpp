...
ValuedAction DESPOT::Search() {
	...
	logi << "[DESPOT::Search] Time for sampling " << particles.size()
		<< " particles: " << (get_time_second() - start) << "s" ;
	...
	logi << "[DESPOT::Search] Time for deleting tree: "
		<< (get_time_second() - start) << "s" ;
	logi << "[DESPOT::Search] Search statistics:"  << statistics_
		;
	...
	do {
		...
		if (double(num_trials - prev_num) > 0.05 * prev_num) {
			...
			cout << "# trials = " << num_trials << "; target = "
				...
				<< root->upper_bound() << "; time = " << used_time ;
...
void DESPOT::Backup(VNode* vnode) {
	int iter = 0;
	logd << "- Backup " << vnode << " at depth " << vnode->depth() ;
	while (true) {
...
void DESPOT::belief(Belief* b) {
	logi << "[DESPOT::belief] Start: Set initial belief." ;
	belief_ = b;
	history_.Truncate(0);

	lower_bound_->belief(b); // needed for POMCPScenarioLowerBound
	logi << "[DESPOT::belief] End: Set initial belief." ;
}
