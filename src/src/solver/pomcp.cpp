...
ValuedAction POMCP::Search(double timeout) {
	...
	while (true) {
		...
		for (int i = 0; i < particles.size(); i++) {
			...
			logd << "[POMCP::Search] Starting simulation " << num_sims ;

			Simulate(particle, root_, model_, prior_);
 
			num_sims++;
			logd << "[POMCP::Search] " << num_sims << " simulations done" ;
	...
	logi << "[POMCP::Search] Search statistics" 
		<< "OptimalAction = " << astar  
		<< "# Simulations = " << root_->count() 
		<< "Time: CPU / Real = " << ((clock() - start_cpu) / CLOCKS_PER_SEC) << " / " << (get_time_second() - start_real) 
		<< "# active particles = " << model_->NumActiveParticles() 
		<< "Tree size = " << root_->Size() ;

	if (astar.action == -1) {
		for (int action = 0; action < model_->NumActions(); action++) {
			cout << "action " << action << ": " << root_->Child(action)->count()
				<< " " << root_->Child(action)->value() ;
		}
	}
...
int POMCP::UpperBoundAction(const VNode* vnode, double explore_constant) {
	...
	/*
	 ...
	 cout << action << " " << ub << " " << qnodes[action]->value() << " " << qnodes[action]->count() << " " << vnode->count() ;
	 }
	 */
...
ValuedAction POMCP::OptimalAction(const VNode* vnode) {
	...
	for (int action = 0; action < qnodes.size(); action++) {
		// cout << action << " " << qnodes[action]->value() << " " << qnodes[action]->count() << " " << vnode->count() ;
...
double POMCP::Rollout(State* particle, RandomStreams& streams, int depth,
	...
	logd << *particle ;
	logd << "depth = " << depth << "; action = " << action ;
...
ValuedAction DPOMCP::Search(double timeout) {
	...
	logi << "[DPOMCP::Search] Time: CPU / Real = "
		...
		<< (get_time_second() - start_real)  << "Tree size = "
		<< root_->Size() ;
