...
void EvalLog::Save() {
	...
	fout << start_time ;
	fout << runned_instances.size() ;
	for (int i = 0; i < runned_instances.size(); i++)
		fout << runned_instances[i] << " " << num_of_completed_runs[i] ;
	fout.close();
}
...
//observation
	...
	*out_ << ";";
	if (state_ != NULL) {
		if (!Globals::config.silence && out_)
			*out_ << model_->ObsProb(obs, *state_, action);
	}
	*out_ << ";";
	ReportStepReward();
	end_t = get_time_second();
 *out_ << endl;
	...
	end_t = get_time_second();
	logi << "[RunStep] Time spent in Update(): " << (end_t - start_t) ;
...
int IPPCEvaluator::Handshake(string instance) {
	...
	if (!Globals::config.silence && out_) {
		*out_ << "Time for handsake " << (end_t - start_t) ;
	}
...
double IPPCEvaluator::EndRound() {
	...
	if (!Globals::config.silence && out_) {
		*out_ << "Total undiscounted reward = " << round_reward ;
	}
...
bool IPPCEvaluator::ExecuteAction(int action, double& reward, OBS_TYPE& obs) {
	...
	if (!Globals::config.silence && out_) {
		*out_ << "Time for executing action " << (end_t - start_t) ;
	}
...
double IPPCEvaluator::End() {
	...
	if (!Globals::config.silence && out_) {
		*out_ << "Time for endsession " << (end_t - start_t)
			<< "Total reward for all runs = " << total_reward
			<< "Total time: Real / CPU = "
			<< (get_time_second() - EvalLog::curr_inst_start_time) << " / "
			<< (double(clock() - start_clockt_) / CLOCKS_PER_SEC) << "s"
			;
...
void IPPCEvaluator::UpdateTimePerMove(double step_time) {
	...
	if (!Globals::config.silence && out_) {
		*out_
			...
			<< (get_time_second() - EvalLog::start_time) ;
	}
}
...
void POMDPEvaluator::InitRound() {
	...
	logi << "[POMDPEvaluator::InitRound] Created start state." ;
	// if (!Globals::config.silence && out_) {
	// 	*out_ << "Initial state: " ;
	// 	model_->PrintState(*state_, *out_);
	// 	*out_ ;
